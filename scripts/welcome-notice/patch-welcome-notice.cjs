'use strict'
/**
 * 首次进入 GUI 的内测声明弹窗（DSH_SHOW_WELCOME_NOTICE=1 可恢复）。
 *
 * 默认跳过：主机端把 __DSH_SKIP_WELCOME_NOTICE__ 注入页面全局（dsh-client-modules），
 * 客户端读到 true 时不再渲染弹窗（dsh-client-ui-settings-models）。
 *
...
const NAME = 'patch-welcome-notice'
