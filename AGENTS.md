...
本仓库对 dsh 移动端（手机 / 窄视口）UI 的定制，均通过在构建时注入 CSS 媒体查询（类名从各包自身 css map 解析，哈希无关）。既有规范：
...
- **给 GUI 加一条移动端 CSS 定制**：确认断点 → 新建 `scripts/<功能>/patch-<功能>.cjs`，用 `hideOnMobile` / `appendCssSuffix` → 跑脚本 → 刷新 GUI 验证 → 幂等复检 → 登记 `docs/scripts.md` 与 `Dockerfile` → 更新 `README.md` + 本文件第 4 节。
